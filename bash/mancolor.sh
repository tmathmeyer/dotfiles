...
#       change this:
export MANCOLORPATH=$HOME/.bash/mancolor.sh
#If you want it to use just the man section as the seed change this
# to 1. Man page of the same secion will have the same schemes.
export USESECTION=1
...
[[ $- == *i* ]] && b6color(){
    ...
    echo -n "$((16#$(MANPAGER=cat /bin/man -w "$@" 2>/dev/null | grep -Poh "(?<=/)[^/]+\.[^\\\.]+(?=\.gz\$)" | md5sum | head -c 4)/2))">/tmp/mancolorseed
    if ((USESECTION==1)); then
        local sec="$(MANPAGER=cat /bin/man -w "$@" 2>/dev/null | grep -Pohm 1 '\d(?=\w*?.gz)')"
        [[ -n "$sec" ]]&& echo -n "$sec">/tmp/mancolorseed
        echo -n "$sec">random
    fi
